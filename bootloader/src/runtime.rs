...
use crate::uefi::{Guid, UefiStatus};
...
pub struct RuntimeServices {
    ...
    pub get_time:
        extern "win64" fn(time: &mut Time, capabilities: *mut TimeCapabilities) -> UefiStatus,
    pub set_time: extern "win64" fn(time: &Time) -> UefiStatus,
    pub get_wakeup_time:
        extern "win64" fn(enabled: &mut bool, pending: &mut bool, time: &mut Time) -> UefiStatus,
    pub set_wakeup_time: extern "win64" fn(enabled: bool, time: &mut Time) -> UefiStatus,
    pub set_virtual_address_map: extern "win64" fn(
    ...
    pub convert_pointer:
        extern "win64" fn(debug_disposition: usize, address: &mut usize) -> UefiStatus,
    ...
    pub get_next_variable_name:
        extern "win64" fn(name_size: &mut usize, name: *mut u16, vendor_guid: &mut Guid)
            -> UefiStatus,
    ...
    pub reset_system: extern "win64" fn(
        rest_type: ResetType,
        reset_status: UefiStatus,
        data_size: usize,
        data: *const u8,
    ) -> !,
    pub update_capsule: extern "win64" fn(
        header_array: *const *const CapsuleHeader,
        count: usize,
    ...
    pub query_variable_info: extern "win64" fn(
        attributes: u32,
        maximum_storage_size: &mut u64,
        remaining_storage_size: &mut u64,
