...
pub static PIC_PAIR: Mutex<PicPair> =
    Mutex::new(unsafe { PicPair::new(LEGACY_PIC_BASE, LEGACY_PIC_BASE + 8) });
...
impl PicPair {
    ...
    pub unsafe fn remap(&mut self) {
        ...
        self.0.command_port.write(0x11);
        wait();
        self.1.command_port.write(0x11);
        wait();

        // Tell the PICs their interrupt vector offsets
        self.0.data_port.write(self.0.vector_offset);
        wait();
        self.1.data_port.write(self.1.vector_offset);
        wait();

        // Tell the master PIC that the slave is at IRQ2 (0000 0100)
        self.0.data_port.write(0x4);
        wait();

        // Tell the slave its cascade identity (0000 0010)
        self.1.data_port.write(0x2);
        wait();

        // Tell the PICs to be in 8086/88 MCS-80/85 mode
        self.0.data_port.write(0x1);
        wait();
        self.1.data_port.write(0x1);
        wait();
