...
impl StringTable {
    pub fn section_name(&self, section: &ElfSection) -> &'static str {
        let name_ptr = unsafe { (&self.0 as *const u8).offset(section.name_index as isize) };
    ...
    fn next(&mut self) -> Option<&'static ElfSection> {
        if self.remaining_sections == 0 {
            ...
            let next_section_addr =
                (self.current_section as *const _ as u64) + u64::from(self.entry_size);
