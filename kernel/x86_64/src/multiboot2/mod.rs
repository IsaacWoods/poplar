...
pub use self::elf_sections::{
    ElfSection, ElfSectionFlags, ElfSectionIter, ElfSectionType, ElfSectionsTag, StringTable,
};
pub use self::framebuffer_tag::FramebufferTag;
use self::header::{Tag, TagIter};
pub use self::memory_map::{MemoryArea, MemoryAreaIter, MemoryMapTag};
...
impl BootInformation {
    ...
    pub fn rsdp(&self) -> Option<&'static RsdpTag> {
        self.tag(14)
            .map(|tag| unsafe { &*(tag as *const Tag as *const RsdpTag) })
    }
    ...
    fn has_valid_end_tag(&self) -> bool {
        ...
        let end_tag_addr =
            multiboot_ptr as usize + (self.multiboot_struct.total_size - END_TAG.size) as usize;
