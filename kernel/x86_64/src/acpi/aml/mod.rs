...
impl AmlParser {
    ...
    fn parse_term_obj(&mut self, acpi_info: &mut AcpiInfo) -> Result<TermObj, AmlError> {
        ...
        match self.stream.next().unwrap() {
            opcodes::SCOPE_OP => self.parse_scope_op(acpi_info),
    ...
    fn parse_term_arg(&mut self) -> Result<TermArg, AmlError> {
         ...
         */
        match self.stream.next().unwrap() {
            opcodes::BYTE_PREFIX => Ok(TermArg::ByteData(self.stream.next().unwrap())),
            ...
            opcodes::ZERO_OP => Ok(TermArg::ZeroOp),

            opcodes::ONE_OP => Ok(TermArg::OneOp),

            opcodes::ONES_OP => Ok(TermArg::OnesOp),
    ...
    fn parse_name_string(&mut self) -> Result<String, AmlError> {
        ...
        match first_byte {
            ...
            _ => {
                 ...
                 */
                let mut name =
                    String::from(str::from_utf8(&[self.stream.next().unwrap()]).unwrap());
                name += &self.parse_name_path()?;
                Ok(name)
            }
        }
