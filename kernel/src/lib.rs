...
// use handle::{Handle, HandleManager, HandleOwner};
    ...
    A: Architecture,
{
    ...
    // let kernel_handle = Handle::new(handle_manager.allocate(), HandleOwner::Kernel);
    ...
    // let test_process_id = process_manager.allocate();
    // let test_process_image = file_manager.open("/ramdisk/test_process.elf").unwrap();
    // let test_process = architecture.create_process(test_process_id, &test_process_image);
